...
@AllArgsConstructor
public class CommentOfUserMatcher implements RequestMatcher {
  ...
  public boolean matches(HttpServletRequest request) {
    ...
    String commentIdFromRequest = getSelfId(request, "comments");
    if (!antMatcher.matches(request)) {
      return false;
    }
    return userService.checkUserOfComment(user.getId(), commentIdFromRequest);
  }
